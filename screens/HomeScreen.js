...
export default class HomeScreen extends React.Component {
  ...
  finishLoginOrRegister = async () => {
    ...
    const greetingMessage = firebase.functions().httpsCallable('greetingMessage')

    const result = await greetingMessage({ message: "!greet" })
      // Sign-out successful.
      console.log("you succeduly signed out")
      await db.collection('users').doc(this.state.email).update({ online: false })
    } catch(error) {
      // An error happened.
      ...
      <View style={styles.container}>
        <View style={styles.contentContainer}>
          <View style={styles.welcomeContainer}>
            ...
            <Button onPress={this.finishLoginOrRegister} title="SIGN OUT" style={{ width: 100, paddingTop: 20 }} />
          </View>
        </View>
